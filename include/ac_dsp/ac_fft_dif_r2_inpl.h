...
//                                                                                  
// CatWare   ac_fft_dif_r2_inpl  is  C_view  of  fft_dif_r2_inpl_core  it  works  as
// C++  wrapper  around generic C Core class 'fft_dif_r2_inpl_core'.                
// 'fft_dif_r2_inpl_core'                                                           
// instantiate butterfly as 'fft_dif_r2_inpl_butterfly' and Compute FFT Flow Graph  
// fetching and writing Data from two Memory Banks 'fft_dif_r2_inpl_butterfly'      
// implements  Radix-2 Butterfly. Butterfly output can  scale-down by 1/2 using     
// bit pattern of stage_scaling parameter                                           
//                                                                                  
...
#ifndef __AC_FFT_DIF_R2_INPL_H
#define __AC_FFT_DIF_R2_INPL_H
...
/*****************************************************************************************/
/* class "fft_dif_r2_inpl_core " implements Fast Fourier Transform (FFT) as               *
 * Decimation                                                                             *
 * In Frequency (DIF) Radix-2 (R2) in In-place (inpl) Architecture fft_dif_r2_inpl_core   *
 * instantiate 1 instance of stages (fft_dif_r2_inpl_butterfly)                           *
 * class has member functions--                                                           *
 * swap() -- Swaps Data b/w two banks                                                     *
 *                                                                                        *
 * fftDifR2InplCore() -- Implements core functionality the FFT.                           *
 *                                                                                        *
...
class fft_dif_r2_inpl_butterfly
{
...
private:
  ...
  //typedef butterfly_params<dType, cx_dType, cx_dround, cx_b_dround, cx_mType, cx_tType> bfly_T_pack;
  ...
  /*
   * FFT computation will be done in 'fftDftR2InplCore'
   */
    ...
    STAGE_LOOP:for (int i = FFT_STAGES - 1; i >= 0; i--) {
      ...
      SEGMENT_LOOP:for (int j = 0; j < N_FFT / 2; j++) {
        ...
        BUTTERFLY_LOOP:for (int kk = 0; kk < N_FFT / 2; kk++) {
           ...
           */
          fft_dif_r2_inpl_butterfly < dType, cx_dType, cx_dround, cx_b_dround, cx_mType, cx_tType > btrfly;
...
private:
  ...
  fft_dif_r2_inpl_core < N_FFT, TWIDDLE_PREC, DIF_D_P, DIF_D_I > fft;    /* instantiate object of fft_dif_r2_inpl_core class */
  dif_input bank_1[N_FFT / 2], bank_2[N_FFT / 2];

public:

  ac_fft_dif_r2_inpl() { //Constructor
