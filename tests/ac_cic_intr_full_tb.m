...
M = 2;
N = 5;
% Create a CIC interpolator object
cicIntr = dsp.CICInterpolator(R, M, N);
...
% outW = ac::log2_ceil<power<R, N - 1>::value*power<M, N>::value>::val + W + int(!S)
outW = ceil(log2((R^(N-1))*(M^N))) + inW;
outI = outW - (inW - inI);
...
output_CIC = cicIntr(sincombine_fp');

% Uncomment this section if you'd like to generate plots.
%{
fvtool(cicIntr, 'NormalizedFrequency', 'on');
...
dlmwrite('ac_cic_intr_full_input.txt', (double(sincombine_fp))', 'delimiter', '', 'precision', 128);
dlmwrite('ac_cic_intr_full_ref.txt', double(output_CIC), 'delimiter', '', 'precision', 128);

% Update the testbench parameter header with the values used in matlab
str_param = ...
...
fID = fopen('ac_cic_intr_full_param.h', 'w');
